...
ox.settings.useful_tags_path = ['highway','ref','maxspeed','lanes','oneway','lit','name','operator','maxspeed:type','highways_england:area','bicycle','surface','source:maxspeed','sidewalk','layer','bridge','junction','carriageway_ref','foot','int_ref','maintenance','source:name','horse','gritting','source:ref','turn:lanes','cycleway','salting:ref','motor_vehicle','description','old_ref','lanes:forward','gritting_operator','gritting_route_ref','alt_name','cycleway:left','parking:lane:both','traffic_restriction','lanes:backward','maxspeed:variable']

TRUNKROADS = _osm_network_download(POLYGON, 'way["highway"]', '["highway"~"motorway|trunk"]')
...
RAILWAY = _osm_network_download(POLYGON, 'way["railway"]', '["railway"!~"highway|cycleway|footway|path|pedestrian|steps|corridor|elevator|escalator|proposed|construction|bridleway|abandoned|platform|raceway|service"]["railway"~"rail|subway|light_rail|tram|narrow_gauge"]')

log('Create node and path data')
NODES = {}
PATHS = {}
for osm_data in TRUNKROADS:
    for e in osm_data['elements']:
        ...
        if e['type'] == 'way': #osm calls network paths 'ways'
            key = e['id']
            PATHS[key] = {**get_path(e), **{'class': 'highway'}}
...
if SHP:
    ...
    GF1.fillna('').to_file('shp/full_ways_gb.shp', crs='epsg:27700')
    GF2.fillna('').to_file('shp/full_nodes_gb.shp', crs='epsg:27700')
...
with open('output-all.geojson', 'w') as fout:
